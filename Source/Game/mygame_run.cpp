...
{
	if ( ( chest_and_key.Top() >= character.Top() && chest_and_key.Top() - chest_and_key.Height() <= character.Top() ) &&
			chest_and_key.Left() <= character.Left() && chest_and_key.Left()+chest_and_key.Width() >= character.Left()  ){
		chest_and_key.SelectShowBitmap(1);
	}
	
	if (phase == 5) {
		if ((door[0].Top() >= character.Top() && door[0].Top() - door[0].Height() <= character.Top()) &&
				door[0].Left() <= character.Left() && door[0].Left() + door[0].Width() >= character.Left()  ){
				door[0].SelectShowBitmap(1);
			}
			if ((door[1].Top() >= character.Top() && door[1].Top() - door[1].Height() <= character.Top()) &&
				door[1].Left() <= character.Left() && door[1].Left() + door[1].Width() >= character.Left()) {
				door[1].SelectShowBitmap(1);
			} 
			if ((door[2].Top() >= character.Top() && door[2].Top() - door[2].Height() <= character.Top()) &&
				door[2].Left() <= character.Left() && door[2].Left() + door[2].Width() >= character.Left()) {
...
void CGameStateRun::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	if (nChar == VK_LEFT) {
		character.SetTopLeft(character.Left() - 50, character.Top() + 1);
	}
}
...
void CGameStateRun::show_text_by_phase() {
	...
	CTextDraw::ChangeFontLog(pDC, fp, 16, "微軟正黑體", RGB(0, 0, 0), 800);
