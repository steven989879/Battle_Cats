...
void game_framework::CGameStateRun_2::OnInit()
{
	...
	character_call_cat_2.LoadBitmapByString({
		"resources/call_cat_2_2.bmp", "resources/call_cat_2_1.bmp" , "resources/call_cat_2_load.bmp"       // 載入召喚貓咪2(空)按鈕
		}, RGB(255, 255, 255));
...
void CGameStateRun_2::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) // 按esc快速返回
{
	if (nChar == VK_ESCAPE) {
...
void CGameStateRun_2::OnLButtonDown(UINT nFlags, CPoint point)  // 處理滑鼠的動作
{
	///////////////////////////////////
	// 藉由點擊次數生成相應數量貓咪
	///////////////////////////////////
	if (point.x >= 470 && point.x <= 614 && point.y >= 680 && point.y <= 789 && money_30 >= base_1.get_price() && cat_1_cool.GetFrameIndexOfBitmap() == 24) {
	...
	else if (point.x >= 625 && point.x <= 769 && point.y >= 680 && point.y <= 789 && money_30 >= base_2.get_price() && cat_2_cool.GetFrameIndexOfBitmap() == 24) {
		...
		cat_one_friend.push_back(temp1);
		cat_one_friend[cat_one_friend.size() - 1].LoadBitmapByString({
		"resources/cat_2_attack_1.bmp" , "resources/cat_2_attack_2.bmp" , "resources/cat_2_attack_1.bmp" , "resources/cat_2_attack_2.bmp"        // 載入貓咪1走路動畫
			}, RGB(255, 255, 255));
		cat_one_friend[cat_one_friend.size() - 1].SetTopLeft(1400, 430);
		cat_one_friend[cat_one_friend.size() - 1].SetAnimation(125, 0);
...
void CGameStateRun_2::OnLButtonUp(UINT nFlags, CPoint point)	// 處理滑鼠的動作
{
}

void CGameStateRun_2::OnMouseMove(UINT nFlags, CPoint point)	// 處理滑鼠的動作
{
}

void CGameStateRun_2::OnRButtonDown(UINT nFlags, CPoint point)  // 處理滑鼠的動作
{
}

void CGameStateRun_2::OnRButtonUp(UINT nFlags, CPoint point)	// 處理滑鼠的動作
{
}
...
void CGameStateRun_2::OnShow()
{
	...
	if (cat_2_cool.GetFrameIndexOfBitmap() > 23) {        // 顯示招喚貓咪1按鈕與冷卻
...
void CGameStateRun_2::draw_text() {
	int Px = 1390;
